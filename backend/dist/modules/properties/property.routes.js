...
router.get('/', async (req, res) => {
    try {
        const { city, area, propertyType, minPrice, maxPrice, minRooms, minBathrooms, searchQuery, sortBy, } = req.query;
        const where = {
        ...
        let properties = await prisma_1.prisma.property.findMany({
            where,
            orderBy: { createdAt: 'desc' },
        });
        if (searchQuery && String(searchQuery).trim().length > 0) {
            const q = String(searchQuery).toLowerCase().trim();
            properties = properties.filter((p) => p.title.toLowerCase().includes(q) ||
...
router.get('/provider/analytics', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const dbInquiriesCount = await prisma_1.prisma.inquiry.count({
            where: { providerId: userId },
        });
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const providerUser = await prisma_1.prisma.user.findUnique({ where: { id: userId } });
        if (!providerUser)
        ...
        try {
            ...
            if (allUsers.length > 0) {
                ...
                (0, email_service_1.sendNewPropertyEmailAlert)(emails, {
                    title,
                    description,
                    propertyType,
                    price: Number(price),
...
router.patch('/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { title, description, propertyType, price, rentalPeriod, rooms, bathrooms, city, area, neighborhood, addressDetails, images, amenities, availability, } = req.body;
...
router.patch('/:id/availability', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
