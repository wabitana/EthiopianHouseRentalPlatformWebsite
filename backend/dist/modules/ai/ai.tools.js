    ...
    switch (name) {
        case 'search_properties': {
            ...
            if (args.area && args.area !== 'All')
                where.area = { contains: args.area, mode: 'insensitive' };
            if (args.neighborhood)
                where.neighborhood = { contains: args.neighborhood, mode: 'insensitive' };
            if (args.propertyType && args.propertyType !== 'All')
                where.propertyType = { equals: args.propertyType, mode: 'insensitive' };
            if (args.minPrice)
                where.price = { ...where.price, gte: Number(args.minPrice) };
            if (args.maxPrice)
                where.price = { ...where.price, lte: Number(args.maxPrice) };
            if (args.bedrooms)
                where.rooms = { gte: Number(args.bedrooms) };
            if (args.bathrooms)
                where.bathrooms = { gte: Number(args.bathrooms) };
            if (args.availability !== undefined)
                where.availability = args.availability;
            const results = await prisma_1.prisma.property.findMany({ where, take: 10, orderBy: { createdAt: 'desc' } });
            return { count: results.length, properties: results.map(formatPropertyRecord) };
        }
        case 'get_property_details': {
            const p = await prisma_1.prisma.property.findUnique({ where: { id: args.propertyId } });
            if (!p)
                return { error: `Property with ID ${args.propertyId} not found in database.` };
        ...
        case 'recommend_properties': {
            ...
            if (args.area)
                where.area = { contains: args.area, mode: 'insensitive' };
            if (args.maxPrice)
                where.price = { lte: Number(args.maxPrice) };
            if (args.bedrooms)
                where.rooms = { gte: Number(args.bedrooms) };
            const properties = await prisma_1.prisma.property.findMany({ where, take: 5, orderBy: { createdAt: 'desc' } });
        ...
        case 'similar_properties': {
            const target = await prisma_1.prisma.property.findUnique({ where: { id: args.propertyId } });
            if (!target)
                return { error: `Target property ${args.propertyId} not found.` };
            const similars = await prisma_1.prisma.property.findMany({
                where: {
                    id: { not: target.id },
        ...
        case 'search_by_budget': {
            const where = { listingStatus: 'active', price: { lte: Number(args.maxPrice) } };
            if (args.minPrice)
                where.price.gte = Number(args.minPrice);
            if (args.city)
                where.city = { contains: args.city, mode: 'insensitive' };
            const properties = await prisma_1.prisma.property.findMany({ where, take: 6, orderBy: { price: 'asc' } });
        ...
        case 'search_by_location': {
            ...
            if (args.maxPrice)
                where.price = { lte: Number(args.maxPrice) };
            const properties = await prisma_1.prisma.property.findMany({ where, take: 6 });
        ...
        case 'search_by_amenities': {
            const properties = await prisma_1.prisma.property.findMany({ where: { listingStatus: 'active' }, take: 10 });
            const filtered = properties.filter((p) => {
                const pAmenities = typeof p.amenities === 'string' ? JSON.parse(p.amenities) : p.amenities;
                return args.amenities.every((reqAmenity) => pAmenities.some((a) => a.toLowerCase().includes(reqAmenity.toLowerCase())));
            });
        ...
        case 'search_by_rooms': {
            const where = { listingStatus: 'active', rooms: { gte: Number(args.bedrooms) } };
            if (args.bathrooms)
                where.bathrooms = { gte: Number(args.bathrooms) };
            if (args.city)
                where.city = { contains: args.city, mode: 'insensitive' };
        ...
        case 'search_available_now': {
            const where = { listingStatus: 'active', availability: true };
            if (args.city)
                where.city = { contains: args.city, mode: 'insensitive' };
            if (args.maxPrice)
                where.price = { lte: Number(args.maxPrice) };
        ...
        case 'get_favorites': {
            if (!ctx.userId)
                return { error: 'Authentication required to view favorites.' };
        ...
        case 'get_user_inquiries': {
            if (!ctx.userId)
                return { error: 'Authentication required to view inquiries.' };
        ...
        case 'get_my_properties': {
            if (!ctx.userId)
                return { error: 'Authentication required for house providers.' };
            const properties = await prisma_1.prisma.property.findMany({
        ...
        case 'summarize_my_listings': {
            if (!ctx.userId)
                return { error: 'Authentication required for house providers.' };
            const properties = await prisma_1.prisma.property.findMany({ where: { providerId: ctx.userId } });
        ...
        case 'get_property_inquiries': {
            if (!ctx.userId)
                return { error: 'Authentication required for house providers.' };
            const where = { providerId: ctx.userId };
            if (args.propertyId)
                where.propertyId = args.propertyId;
            const inquiries = await prisma_1.prisma.inquiry.findMany({ where, orderBy: { createdAt: 'desc' } });
            return { count: inquiries.length, inquiries };
        }
        ...
        case 'listing_improvement_suggestions': {
            ...
            const suggestions = [];
            if (images.length < 3)
                suggestions.push('Add at least 3 to 5 high-resolution photos of rooms.');
            if (p.description.length < 50)
                suggestions.push('Expand property description detailing water availability & neighborhood features.');
            if (!p.addressDetails)
                suggestions.push('Provide clear landmark directions in address details.');
        ...
        case 'instant_book_tour': {
            ...
            if (!p)
                return { error: `Property ${args.propertyId} not found.` };
            if (ctx.userId) {
                await prisma_1.prisma.inquiry.create({
                    data: {
                        propertyId: p.id,
                        propertyTitle: p.title,
        ...
        case 'market_price_analyzer': {
            ...
            if (priceDiff < -3000)
                dealRating = '🔥 Great Bargain Deal (Below Subcity Average)';
            else if (priceDiff > 5000)
                dealRating = 'Premium / Above Average';
            return {
                propertyTitle: p.title,
