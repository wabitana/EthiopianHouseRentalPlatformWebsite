  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 py-10 px-4">
      <div className="mx-auto max-w-5xl">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-2">Purchase Offers</h1>
        <p className="text-slate-500 dark:text-slate-400 mb-8">Manage offers sent by potential buyers for properties you listed for sale.</p>
            ...
            {requests.map((r) => (
              <div key={r.id} className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 p-6 rounded-2xl shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Buyer Name: <span className="font-semibold text-slate-700 dark:text-slate-300">{r.buyer?.name}</span> ({r.buyer?.email})</p>
                ...
                <div className="flex items-center gap-3.5">
                  {r.status === 'PENDING' ? (
                    ...
                    <span className={`px-3 py-1 rounded-full font-bold text-xs ${
                      r.status === 'ACCEPTED' ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-400' : 'bg-rose-100 text-rose-800 dark:bg-rose-950/40 dark:text-rose-450'
                    }`}>
                      {r.status}
                    </span>
                  )}
