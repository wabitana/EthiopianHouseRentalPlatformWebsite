  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 py-10 px-4">
      <div className="mx-auto max-w-5xl">
        <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-2">My Offers & Purchase Requests</h1>
        <p className="text-slate-500 dark:text-slate-400 mb-8">Track status of offers sent to property sellers.</p>
            ...
            {requests.map((r) => (
              <div key={r.id} className="bg-white dark:bg-slate-900 border border-slate-150 dark:border-slate-800 p-6 rounded-2xl shadow-sm flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Seller: <span className="font-semibold">{r.owner?.name || 'Owner'}</span> ({r.owner?.email})</p>
